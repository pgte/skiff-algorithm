...
N.load = function load(cb) {
  ...
  function newPeer(peerDesc) {
    var peer = new Peer(
      peerDesc.id, peerDesc.metadata, self.options, null, self);
    self._join(peer);
...
N.listen = function listen(options, cb) {
  ...
  function listener(peerId, metadata, connection) {
    self._join(peerId, metadata, connection);
  }
};

/// Peers

N.join = function join(peerDesc, metadata, cb) {
  var self = this;
  ...
  function pushAddPeerCommand(cb) {
    var cmd = ['add peer', peerDesc, metadata];
...
N.peerMeta = function peerMeta(url) {
...
N._join = function _join(peerDesc, metadata, connection) {
  var self = this;
  ...
  if (peerDesc != this.id) {
    ...
    if (!(peerDesc instanceof Peer)) {
      peer = new Peer(peerDesc, metadata, this.options, connection, self);
    }
    if (found) {
      this.commonState.persisted.peers[i] = peer;
    }
    else {
  ...
  function onPeerConnecting() {
    self.emit('connecting', peer);
  }
};
...
N.toState = function toState(state) {
  ...
  self.state.on('warning', function(warn) {
    self.emit('warning', warn);
  });
...
N.ensureLeader = function ensureLeader(cb) {
  ...
  if (this.state.name != 'leader') {
    ...
    err.code = 'ENOTLEADER';
    err.leader = this.commonState.volatile.leaderId;
  }
  cb(err);
};

N.currentLeader = function() {
  return this.commonState.volatile.leaderId;
};
...
N.command = function command(cmd, options, cb, isTopologyChange) {
  ...
  function replicateAgain(cb) {
    commitIndex = self.commonState.persisted.log.length();
    self.state.replicate(commitIndex, options.timeout, options.waitForNode, cb);
  }
};
...
N.handlePeerCall = function handlePeerCall(peer, type, args, cb) {
  ...
  function handlerReplied() {
    ...
    if (requestArgs.term && requestArgs.term > self.currentTerm()) {
